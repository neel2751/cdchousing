  ...
  return (
    <Link href={`/projects/${project.id}`}>
      ...
      >
        ...
        <div className={`absolute top-4 left-4 text-white text-xs px-3 py-1 tracking-widest uppercase ${statusBg}`}>
          {project.status}
        </div>

        <div className="absolute top-4 right-4 bg-light/90 text-primary text-xs px-3 py-1 tracking-widest uppercase">
          {project.year}
        </div>
        ...
        >
          ...
          <span className="inline-flex items-center gap-3 border border-secondary text-secondary text-xs tracking-widest uppercase px-7 py-3 rounded-full group-hover:bg-secondary group-hover:text-white transition-all duration-500">
            Explore Project →
          </span>
        </div>
      </div>
